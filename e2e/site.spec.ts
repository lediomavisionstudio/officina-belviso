...
test.describe('Home V2', () => {
  ...
  test('renders the premium footer with business information and clickable contact details', async ({ page }) => {
    ...
    await expect(footer.getByText('Viale Sindaco Gerardo Decaro 9/11, Zona P.I.P.')).toBeVisible()
  ...
  test('presents workshop details, Google review controls, and the Maps handoff beside the unchanged quote form', async ({ page }) => {
    ...
    await expect(section.getByText('Viale Sindaco Gerardo Decaro 9/11, Zona P.I.P.')).toHaveCount(0)
  ...
  test.describe('touch pagination', () => {
    ...
    test('keeps both carousel axes independent from page scrolling', async ({
      page,
      context,
    }) => {
      await page.goto('/')
      ...
      for (const destination of [
        {
          label: 'Servizi Officina Belviso',
          navigate: () => openJourneyPanel(page, 'servizi'),
        },
        {
          ...
          navigate: async () => {
            ...
            await page
              .getByRole('link', { name: 'I nostri lavori', exact: true })
              .click()
          },
        },
      ]) {
        await destination.navigate()
        const track = page
          .getByRole('region', { name: destination.label })
          .locator('[data-carousel-track]')
        await track.scrollIntoViewIfNeeded()

        const verticalBounds = await track.boundingBox()
        expect(verticalBounds).not.toBeNull()
        if (!verticalBounds) continue
        const scrollBeforeVerticalGesture = await page.evaluate(() => window.scrollY)
        const verticalX = verticalBounds.x + verticalBounds.width * 0.5
        const verticalStartY = verticalBounds.y + verticalBounds.height * 0.7
        const verticalEndY = Math.max(
          12,
          verticalBounds.y + verticalBounds.height * 0.35,
        )

        await session.send('Input.dispatchTouchEvent', {
          type: 'touchStart',
          touchPoints: [{ x: verticalX, y: verticalStartY }],
        })
        for (let step = 1; step <= 10; step += 1) {
          await session.send('Input.dispatchTouchEvent', {
            type: 'touchMove',
            touchPoints: [
              {
                ...
                y:
                  verticalStartY +
                  ((verticalEndY - verticalStartY) * step) / 10,
              },
            ],
          })
        }
        ...
        await expect
          .poll(() => page.evaluate(() => window.scrollY))
          .toBeGreaterThan(scrollBeforeVerticalGesture + 20)

        await track.scrollIntoViewIfNeeded()
        const horizontalBounds = await track.boundingBox()
        expect(horizontalBounds).not.toBeNull()
        if (!horizontalBounds) continue
        const horizontalStartX = horizontalBounds.x + horizontalBounds.width * 0.8
        const horizontalEndX = horizontalBounds.x + horizontalBounds.width * 0.2
        const horizontalY = horizontalBounds.y + horizontalBounds.height * 0.5

        await session.send('Input.dispatchTouchEvent', {
          type: 'touchStart',
          touchPoints: [{ x: horizontalStartX, y: horizontalY }],
        })
        for (let step = 1; step <= 10; step += 1) {
          await session.send('Input.dispatchTouchEvent', {
            type: 'touchMove',
            touchPoints: [
              {
                x:
                  horizontalStartX +
                  ((horizontalEndX - horizontalStartX) * step) / 10,
                y: horizontalY,
              },
            ],
          })
